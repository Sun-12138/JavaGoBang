package pers.gobang.view.child.game;
...
public class CGameController extends ViewAlter implements Initializable {
    static SwitchTimer switchTimer = new SwitchTimer(); //实例化定时器线程管理
    @FXML
    public HBox child_pane;
    ...
    private void initView() {
        ...
        timer_pane.setStyle("-fx-background-size:" + timer_pane.getWidth());
        deleteGame.setStyle("-fx-background-size:" + deleteGame.getWidth());
        gotoAdult.setStyle("-fx-background-size:" + gotoAdult.getWidth());

        if (user.getGame_mod()) {
            //单人
            play_one.setText("Player");
            play_two.setText("AI");
            AnchorPane.setLeftAnchor(play_one, 45.0);
            AnchorPane.setRightAnchor(play_two, 90.0);
        } else {
            //双人
            play_one.setText("Player 1");
            play_two.setText("Player 2");
        }
     ...
     */
    private void setMouseListenerClick(Pane pane) {
        pane.setOnMouseClicked(event -> {
            if (switch_MouseListener) {
                ...
                if (game_Mode && temp != -2) {
                    ...
                    int temp_2 = chessBoard.drawWhiteChessUnit(coordinate[0], coordinate[1], StaticResourcesConfig.CHILD_WHITE_CHESS);
                    if (b == 0) {
    ...
    private int drawChess(double i, double j) {
        ...
        if (game_Mode) {
            ...
            isBlack = false;
            return chessBoard.drawBlackChessUnit(chess_Coordinate[0], chess_Coordinate[1], StaticResourcesConfig.CHILD_BLACK_CHESS);
        } else {
            ...
            if (isBlack) {
                ...
                isBlack = true;
                return chessBoard.drawWhiteChessUnit(chess_Coordinate[0], chess_Coordinate[1], StaticResourcesConfig.CHILD_WHITE_CHESS);
            }
        }
    }
}
